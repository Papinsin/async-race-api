...
export default function App() {
  ...
  async function  startSingleCarRace(startedCarID: number | string) {
    ...
    if (!carIntervalsRef.current[startedCarID] && carIntervalsRef.current[startedCarID] !== undefined) {
    ...
    try {
  const res = await fetch(`${coreUrl}/engine?id=${startedCarID}&status=drive`, { method: "PATCH" });
  if (!res.ok) throw new Error(await res.text());
  ...
  return (
    <CarArrayContext.Provider value={{ carsArray, setCarsArray }}>
      <div className="bg-slate-900 text-white flex flex-col justify-between font-sans relative overflow-x-hidden h-svh">
        {showModal && (
          <Modal finishedCarsArray={finishedCarsArray} closeModalFunction={closeModalFunction}></Modal>
        )}
        <Layout>
          <FormCars></FormCars>
